...
class cursos_categorias {
	...
	public function edit($id_categorias){
		global $db, $sesion;
		try{
			$sql= "SELECT * FROM cursos_categorias WHERE id_categorias=:id_categorias AND eliminado_categorias = 0 ";
			$sql = $db->prepare($sql);
				...
				while($row = $sql->fetchObject()) {
					...
					if (count($photo_categorias) > 0) {
						foreach ($photo_categorias as $key => $value) {
							...
							$dir = _HOSTDIR_."/uploads/cursos/categorias/$key/big/".$value;
							$dir2 = _HOSTDIR_."/uploads/cursos/categorias/$key/small/".$value;

							if (file_exists($dir) && file_exists($dir2) && $value != "") {
								$this->photo_categorias[$key]['file'] =_HOST_."/uploads/cursos/categorias/$key/big/".$value;
							}else{
	...
	public function view($seo_categorias){
		global $db, $sesion;
		try{
			$sql= "SELECT * FROM cursos_categorias WHERE seo_categorias=:seo_categorias AND eliminado_categorias = 0 ";
			$sql = $db->prepare($sql);
	...
	public function select($id_categorias = null, $id_lineas = -1){
		...
		try{
			$data = '';
			$sql = " SELECT * FROM cursos_categorias WHERE eliminado_categorias = 0 ";
			if ($id_lineas >= 0) {
	...
	public function data($id_lineas = -1){
		...
		try{
			$data = array();
			$sql= "SELECT * FROM cursos_categorias WHERE active_categorias = '1' AND eliminado_categorias = 0 ";
			if ($id_lineas > 0) {
				$sql .= " AND id_lineas = '".$id_lineas."' ";
			}
			$sql .= " ORDER BY orden_categorias ASC, name_categorias ASC ";
				...
				while($row = $sql->fetchObject()) {
					...
					if (count($photo_categorias) > 0) {
						foreach ($photo_categorias as $key => $value) {
							...
							if (file_exists($dir) && file_exists($dir2) && $value != "") {
								$json['photo'][$key]['file'] =_HOST_."/uploads/cursos/categorias/$key/big/".$value;
							}else{
								$json['photo'][$key]['file'] = '';
							}
						}
	...
	public function update($id_categorias, $name_categorias, $seo_categorias, $active_categorias, $photo_categorias, $id_lineas, $extra_categorias){
		global $db, $sesion;
		try{
			$sql = "UPDATE cursos_categorias SET name_categorias = :name_categorias, seo_categorias = :seo_categorias, active_categorias = :active_categorias, photo_categorias = :photo_categorias, id_lineas = :id_lineas, extra_categorias = :extra_categorias WHERE id_categorias = :id_categorias";
			$stmt = $db->prepare($sql);
	...
	public function add($name_categorias, $seo_categorias, $active_categorias, $photo_categorias, $id_lineas, $extra_categorias){
		global $db, $sesion;
		try{
			$sql= "SELECT * FROM cursos_categorias WHERE name_categorias=:name_categorias AND eliminado_categorias = 0 ";
			$stmt = $db->prepare($sql);
			$stmt->execute(array(":name_categorias"=>$name_categorias));
			if($stmt->rowCount() > 0){
				return false;
			}else{
				$sql = "INSERT INTO cursos_categorias (name_categorias, seo_categorias, active_categorias, photo_categorias, id_lineas, extra_categorias) VALUES(:name_categorias, :seo_categorias, :active_categorias, :photo_categorias, :id_lineas, :extra_categorias)";
				$stmt = $db->prepare($sql);
				$stmt->bindParam(':name_categorias', $name_categorias, PDO::PARAM_STR);
				$stmt->bindParam(':seo_categorias', $seo_categorias, PDO::PARAM_STR);
				$stmt->bindParam(':active_categorias', $active_categorias, PDO::PARAM_STR);
	...
	public function orden($id_categorias, $orden_categorias){
		global $db;
		try{
			$sql = "UPDATE cursos_categorias SET orden_categorias = :orden_categorias WHERE id_categorias = :id_categorias";
			$stmt = $db->prepare($sql);
			$stmt->bindParam(':id_categorias', $id_categorias, PDO::PARAM_STR);
	...
	public function delete($id_categorias){
		...
		$sql = "UPDATE cursos_categorias SET eliminado_categorias = :eliminado_categorias  WHERE id_categorias = :id_categorias";
		$stmt = $db->prepare($sql);
		$stmt->bindParam(':id_categorias', $id_categorias, PDO::PARAM_STR);
...
$cursos_categorias = new cursos_categorias();