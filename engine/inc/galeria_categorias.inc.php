...
class galeria_categorias {
	...
	public function edit($id_categorias){
		global $db, $sesion;
		try{
			$sql= "SELECT * FROM galeria_categorias WHERE id_categorias=:id_categorias AND eliminado_categorias = 0 ";
			$sql = $db->prepare($sql);
			...
			if($sql->rowCount() > 0){
				while($row = $sql->fetchObject()) {
					...
					$dir = _HOSTDIR_."/uploads/galeria/categorias/big/".$row->photo_categorias;
					$dir2 = _HOSTDIR_."/uploads/galeria/categorias/small/".$row->photo_categorias;
					if (file_exists($dir) && file_exists($dir2) && $row->photo_categorias != "") {
						$this->photo_categorias=_HOST_."/uploads/galeria/categorias/small/".$row->photo_categorias;
					}else{
	...
	public function view($seo_categorias){
		global $db, $sesion;
		try{
			$sql= "SELECT * FROM galeria_categorias WHERE seo_categorias=:seo_categorias AND eliminado_categorias = 0 ";
			$sql = $db->prepare($sql);
			...
			if($sql->rowCount() > 0){
				while($row = $sql->fetchObject()) {
					...
					$dir2 = _HOSTDIR_."/uploads/galeria/categorias/small/".$row->photo_categorias;
					if (file_exists($dir) && file_exists($dir2) && $row->photo_categorias != "") {
						$this->photo_categorias=_HOST_."/uploads/galeria/categorias/big/".$row->photo_categorias;
						$this->photo_small_categorias=_HOST_."/uploads/galeria/categorias/small/".$row->photo_categorias;
					}else{
	...
	public function select($id_categorias = null, $id_lineas = -1){
		...
		try{
			$data = '';
			$sql = " SELECT * FROM galeria_categorias WHERE eliminado_categorias = 0 ";
			if ($id_lineas >= 0) {
	...
	public function data(){
		...
		try{
			$data = array();
			$sql= "SELECT * FROM galeria_categorias WHERE active_categorias = '1' AND eliminado_categorias = 0 ORDER BY orden_categorias ASC, name_categorias ASC ";
			$sql = $db->prepare($sql);
			$sql->execute();
			if($sql->rowCount() > 0){
				while($row = $sql->fetchObject()) {
					...
					if (file_exists($dir)  && $row->photo_categorias != "") {
						$json['photo'] =_HOST_."/uploads/galeria/categorias/big/".$row->photo_categorias;
					}else{
					...
					if (file_exists($dir2)  && $row->photo_categorias != "") {
						$json['photo_small'] =_HOST_."/uploads/galeria/categorias/small/".$row->photo_categorias;
					}else{
	...
	public function update($id_categorias, $name_categorias, $seo_categorias, $active_categorias, $photo_categorias, $id_lineas, $extra_categorias){
		global $db, $sesion;
		try{
			$sql = "UPDATE galeria_categorias SET name_categorias = :name_categorias, seo_categorias = :seo_categorias, active_categorias = :active_categorias, photo_categorias = :photo_categorias, id_lineas = :id_lineas, extra_categorias = :extra_categorias WHERE id_categorias = :id_categorias";
			$stmt = $db->prepare($sql);
	...
	public function add($name_categorias, $seo_categorias, $active_categorias, $photo_categorias, $id_lineas, $extra_categorias){
		global $db, $sesion;
		try{
			$sql= "SELECT * FROM galeria_categorias WHERE name_categorias=:name_categorias AND eliminado_categorias = 0 ";
			$stmt = $db->prepare($sql);
			$stmt->execute(array(":name_categorias"=>$name_categorias));
			if($stmt->rowCount() > 0){
				return false;
			}else{
				$sql = "INSERT INTO galeria_categorias (name_categorias, seo_categorias, active_categorias, photo_categorias, id_lineas, extra_categorias) VALUES(:name_categorias, :seo_categorias, :active_categorias, :photo_categorias, :id_lineas, :extra_categorias)";
				$stmt = $db->prepare($sql);
				$stmt->bindParam(':name_categorias', $name_categorias, PDO::PARAM_STR);
				$stmt->bindParam(':seo_categorias', $seo_categorias, PDO::PARAM_STR);
				$stmt->bindParam(':active_categorias', $active_categorias, PDO::PARAM_STR);
	...
	public function orden($id_categorias, $orden_categorias){
		global $db;
		try{
			$sql = "UPDATE galeria_categorias SET orden_categorias = :orden_categorias WHERE id_categorias = :id_categorias";
			$stmt = $db->prepare($sql);
			$stmt->bindParam(':id_categorias', $id_categorias, PDO::PARAM_STR);
	...
	public function delete($id_categorias){
		...
		$sql = "UPDATE galeria_categorias SET eliminado_categorias = :eliminado_categorias  WHERE id_categorias = :id_categorias";
		$stmt = $db->prepare($sql);
		$stmt->bindParam(':id_categorias', $id_categorias, PDO::PARAM_STR);
...
$galeria_categorias = new galeria_categorias();