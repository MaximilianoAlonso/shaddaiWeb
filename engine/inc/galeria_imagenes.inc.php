...
class galeria_imagenes {
	...
	public function tabla($id_galeria = 0, $requestData, $web){
		global $db, $sesion;
		try{
			...
			while($row = $q->fetchObject()) {
				...
				$dir = _HOSTDIR_."/uploads/galeria/imagenes/big/".$row->photo_imagenes;
				$dir2 = _HOSTDIR_."/uploads/galeria/imagenes/small/".$row->photo_imagenes;
				if (file_exists($dir) && file_exists($dir2) && $row->photo_imagenes != "") {
					$nestedData[] = '<img src="'._HOST_.'/uploads/galeria/imagenes/small/'.$row->photo_imagenes.'" alt="'.$row->photo_imagenes.'" style="max-width: 150px;" >';
				}else{
	...
	public function edit($id_imagenes){
		global $db, $sesion;
		try{
			$sql= "SELECT * FROM galeria_imagenes WHERE id_imagenes=:id_imagenes AND eliminado_imagenes = 0 ";
			$sql = $db->prepare($sql);
			...
			if($sql->rowCount() > 0){
				while($row = $sql->fetchObject()) {
					...
					$dir2 = _HOSTDIR_."/uploads/galeria/imagenes/small/".$row->photo_imagenes;
					if (file_exists($dir) && file_exists($dir2) && $row->photo_imagenes != "") {
						$this->photo_imagenes=_HOST_."/uploads/galeria/imagenes/small/".$row->photo_imagenes;
					}else{
	...
	public function select($id_imagenes = null, $id_galeria = -1){
		global $db;
		try{
			$data = '';
			$sql = " SELECT * FROM galeria_imagenes WHERE eliminado_imagenes = 0 ";
			if ($id_galeria >= 0) {
				$sql .= " AND id_galeria = '".$id_galeria."' ";
			}
	...
	public function data($id_galeria = 0, $random = null, $limit = 0){
		global $db, $sesion;
		try{
			$data = array();
			$sql = "SELECT * FROM galeria_imagenes WHERE active_imagenes = '1' AND eliminado_imagenes = 0 ";
			if ($id_galeria > 0) {
				$sql .= " AND id_galeria = '".$id_galeria."' ";
			}
			if ($random == true) {
			...
			if($sql->rowCount() > 0){
				while($row = $sql->fetchObject()) {
					...
					if (file_exists($dir)  && $row->photo_imagenes != "") {
						$json['photo'] =_HOST_."/uploads/galeria/imagenes/big/".$row->photo_imagenes;
					}else{
					...
					if (file_exists($dir2)  && $row->photo_imagenes != "") {
						$json['photo_small'] =_HOST_."/uploads/galeria/imagenes/small/".$row->photo_imagenes;
					}else{
	...
	public function listado($id_galeria = 0, $pagination = 0, $limit = 10){
		global $db, $sesion;
		try{
	...
	public function update($id_imagenes, $name_imagenes, $active_imagenes, $photo_imagenes, $id_galeria, $extra_imagenes){
		global $db, $sesion;
		try{
			$sql = "UPDATE galeria_imagenes SET name_imagenes = :name_imagenes, active_imagenes = :active_imagenes, photo_imagenes = :photo_imagenes, id_galeria = :id_galeria, extra_imagenes = :extra_imagenes WHERE id_imagenes = :id_imagenes";
			$stmt = $db->prepare($sql);
	...
	public function add($name_imagenes, $active_imagenes, $photo_imagenes, $id_galeria, $extra_imagenes){
		global $db, $sesion;
		try{
			$sql= "SELECT * FROM galeria_imagenes WHERE name_imagenes=:name_imagenes AND id_galeria = :id_galeria AND eliminado_imagenes = 0 ";
			$stmt = $db->prepare($sql);
			$stmt->execute(array(":name_imagenes"=>$name_imagenes, ":id_galeria" => $id_galeria));
			if($stmt->rowCount() > 0){
				return false;
			}else{
				$sql = "INSERT INTO galeria_imagenes (name_imagenes, active_imagenes, photo_imagenes, id_galeria, extra_imagenes) VALUES(:name_imagenes, :active_imagenes, :photo_imagenes, :id_galeria, :extra_imagenes)";
				$stmt = $db->prepare($sql);
				$stmt->bindParam(':name_imagenes', $name_imagenes, PDO::PARAM_STR);
				$stmt->bindParam(':active_imagenes', $active_imagenes, PDO::PARAM_STR);
				$stmt->bindParam(':photo_imagenes', $photo_imagenes, PDO::PARAM_STR);
	...
	public function orden($id_imagenes, $orden_imagenes){
		global $db;
		try{
			$sql = "UPDATE galeria_imagenes SET orden_imagenes = :orden_imagenes WHERE id_imagenes = :id_imagenes";
			$stmt = $db->prepare($sql);
			$stmt->bindParam(':id_imagenes', $id_imagenes, PDO::PARAM_STR);
	...
	public function delete($id_imagenes){
		...
		$sql = "UPDATE galeria_imagenes SET eliminado_imagenes = :eliminado_imagenes  WHERE id_imagenes = :id_imagenes";
		$stmt = $db->prepare($sql);
		$stmt->bindParam(':id_imagenes', $id_imagenes, PDO::PARAM_STR);
