...
class cursos_lineas {
	...
	public function tabla($requestData, $web){
		...
		return $datatable->get($requestData, 'cursos_lineas', array('id_lineas', 'name_lineas'), null, array('name_lineas'), 'active_lineas', null, null, null, $web, null, 'eliminado_lineas');
	}

	public function edit($id_lineas){
		global $db, $sesion;
		try{
			$sql= "SELECT * FROM cursos_lineas WHERE id_lineas=:id_lineas AND eliminado_lineas = 0 ";
			$sql = $db->prepare($sql);
			...
			if($sql->rowCount() > 0){
				while($row = $sql->fetchObject()) {
					...
					$dir = _HOSTDIR_."/uploads/cursos/lineas/big/".$row->photo_lineas;
					$dir2 = _HOSTDIR_."/uploads/cursos/lineas/small/".$row->photo_lineas;
					if (file_exists($dir) && file_exists($dir2) && $row->photo_lineas != "") {
						$this->photo_lineas=_HOST_."/uploads/cursos/lineas/small/".$row->photo_lineas;
					}else{
	...
	public function view($seo_lineas){
		global $db, $sesion;
		try{
			$sql= "SELECT * FROM cursos_lineas WHERE seo_lineas=:seo_lineas AND eliminado_lineas = 0 ";
			$sql = $db->prepare($sql);
			...
			if($sql->rowCount() > 0){
				while($row = $sql->fetchObject()) {
					...
					$dir2 = _HOSTDIR_."/uploads/cursos/lineas/small/".$row->photo_lineas;
					if (file_exists($dir) && file_exists($dir2) && $row->photo_lineas != "") {
						$this->photo_lineas=_HOST_."/uploads/cursos/lineas/big/".$row->photo_lineas;
						$this->photo_small_lineas=_HOST_."/uploads/cursos/lineas/small/".$row->photo_lineas;
					}else{
	...
	public function select($id_lineas = null){
		...
		try{
			$data = '';
			$q= "SELECT * FROM cursos_lineas WHERE eliminado_lineas = 0 ORDER BY orden_lineas ASC, name_lineas ASC ";
			$q = $db->prepare($q);
			$q->execute();
			if($q->rowCount() > 0){
	...
	public function data(){
		...
		try{
			$data = array();
			$sql= "SELECT * FROM cursos_lineas WHERE active_lineas = '1' AND eliminado_lineas = 0 ORDER BY orden_lineas ASC, name_lineas ASC ";
			$sql = $db->prepare($sql);
			$sql->execute();
			if($sql->rowCount() > 0){
				while($row = $sql->fetchObject()) {
					...
					if (file_exists($dir)  && $row->photo_lineas != "") {
						$json['photo'] =_HOST_."/uploads/cursos/lineas/big/".$row->photo_lineas;
					}else{
					...
					if (file_exists($dir2)  && $row->photo_lineas != "") {
						$json['photo_small'] =_HOST_."/uploads/cursos/lineas/small/".$row->photo_lineas;
					}else{
	...
	public function update($id_lineas, $name_lineas, $seo_lineas, $active_lineas, $photo_lineas, $extra_lineas){
		global $db, $sesion;
		try{
			$sql = "UPDATE cursos_lineas SET name_lineas = :name_lineas, seo_lineas = :seo_lineas, active_lineas = :active_lineas, photo_lineas = :photo_lineas, extra_lineas = :extra_lineas WHERE id_lineas = :id_lineas";
			$stmt = $db->prepare($sql);
	...
	public function add($name_lineas, $seo_lineas, $active_lineas, $photo_lineas, $extra_lineas){
		global $db, $sesion;
		try{
			$sql= "SELECT * FROM cursos_lineas WHERE name_lineas=:name_lineas AND eliminado_lineas = 0 ";
			$stmt = $db->prepare($sql);
			$stmt->execute(array(":name_lineas"=>$name_lineas));
			if($stmt->rowCount() > 0){
				return false;
			}else{
				$sql = "INSERT INTO cursos_lineas (name_lineas, seo_lineas, active_lineas, photo_lineas, extra_lineas) VALUES(:name_lineas, :seo_lineas, :active_lineas, :photo_lineas, :extra_lineas)";
				$stmt = $db->prepare($sql);
				$stmt->bindParam(':name_lineas', $name_lineas, PDO::PARAM_STR);
				$stmt->bindParam(':seo_lineas', $seo_lineas, PDO::PARAM_STR);
				$stmt->bindParam(':active_lineas', $active_lineas, PDO::PARAM_STR);
	...
	public function orden($id_lineas, $orden_lineas){
		global $db;
		try{
			$sql = "UPDATE cursos_lineas SET orden_lineas = :orden_lineas WHERE id_lineas = :id_lineas";
			$stmt = $db->prepare($sql);
			$stmt->bindParam(':id_lineas', $id_lineas, PDO::PARAM_STR);
	...
	public function delete($id_lineas){
		...
		$sql = "UPDATE cursos_lineas SET eliminado_lineas = :eliminado_lineas  WHERE id_lineas = :id_lineas";
		$stmt = $db->prepare($sql);
		$stmt->bindParam(':id_lineas', $id_lineas, PDO::PARAM_STR);
...
$cursos_lineas = new cursos_lineas();