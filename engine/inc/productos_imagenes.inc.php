...
class productos_imagenes {
	...
	public function tabla($id_productos = 0, $requestData, $web){
		global $db, $sesion;
		try{
			...
			c.*
			FROM
			productos_imagenes c
			WHERE
			c.id_productos = '".$id_productos."' 
			AND
			...
			while($row = $q->fetchObject()) {
				...
				$dir = _HOSTDIR_."/uploads/productos/imagenes/big/".$row->photo_imagenes;
				$dir2 = _HOSTDIR_."/uploads/productos/imagenes/small/".$row->photo_imagenes;
				if (file_exists($dir) && file_exists($dir2) && $row->photo_imagenes != "") {
					$nestedData[] = '<img src="'._HOST_.'/uploads/productos/imagenes/small/'.$row->photo_imagenes.'" alt="'.$row->photo_imagenes.'" style="max-width: 150px;" >';
				}else{
	...
	public function edit($id_imagenes){
		global $db, $sesion;
		try{
			$sql= "SELECT * FROM productos_imagenes WHERE id_imagenes=:id_imagenes AND eliminado_imagenes = 0 ";
			$sql = $db->prepare($sql);
			...
			if($sql->rowCount() > 0){
				while($row = $sql->fetchObject()) {
					...
					$dir2 = _HOSTDIR_."/uploads/productos/imagenes/small/".$row->photo_imagenes;
					if (file_exists($dir) && file_exists($dir2) && $row->photo_imagenes != "") {
						$this->photo_imagenes=_HOST_."/uploads/productos/imagenes/small/".$row->photo_imagenes;
					}else{
	...
	public function select($id_imagenes = null, $id_productos = -1){
		global $db;
		try{
			$data = '';
			$sql = " SELECT * FROM productos_imagenes WHERE eliminado_imagenes = 0 ";
			if ($id_productos >= 0) {
				$sql .= " AND id_productos = '".$id_productos."' ";
			}
	...
	public function data($id_productos = 0, $random = null, $limit = 0){
		global $db, $sesion;
		try{
			$data = array();
			$sql = "SELECT * FROM productos_imagenes WHERE active_imagenes = '1' AND eliminado_imagenes = 0 ";
			if ($id_productos > 0) {
				$sql .= " AND id_productos = '".$id_productos."' ";
			}
			if ($random == true) {
			...
			if($sql->rowCount() > 0){
				while($row = $sql->fetchObject()) {
					...
					if (file_exists($dir)  && $row->photo_imagenes != "") {
						$json['photo'] =_HOST_."/uploads/productos/imagenes/big/".$row->photo_imagenes;
					}else{
					...
					if (file_exists($dir2)  && $row->photo_imagenes != "") {
						$json['photo_small'] =_HOST_."/uploads/productos/imagenes/small/".$row->photo_imagenes;
					}else{
	...
	public function listado($id_productos = 0, $pagination = 0, $limit = 10){
		global $db, $sesion;
		try{
	...
	public function update($id_imagenes, $name_imagenes, $active_imagenes, $photo_imagenes, $id_productos, $extra_imagenes){
		global $db, $sesion;
		try{
			$sql = "UPDATE productos_imagenes SET name_imagenes = :name_imagenes, active_imagenes = :active_imagenes, photo_imagenes = :photo_imagenes, id_productos = :id_productos, extra_imagenes = :extra_imagenes WHERE id_imagenes = :id_imagenes";
			$stmt = $db->prepare($sql);
	...
	public function add($name_imagenes, $active_imagenes, $photo_imagenes, $id_productos, $extra_imagenes){
		global $db, $sesion;
		try{
			$sql= "SELECT * FROM productos_imagenes WHERE name_imagenes=:name_imagenes AND id_productos = :id_productos AND eliminado_imagenes = 0 ";
			$stmt = $db->prepare($sql);
			$stmt->execute(array(":name_imagenes"=>$name_imagenes, ":id_productos" => $id_productos));
			if($stmt->rowCount() > 0){
				return false;
			}else{
				$sql = "INSERT INTO productos_imagenes (name_imagenes, active_imagenes, photo_imagenes, id_productos, extra_imagenes) VALUES(:name_imagenes, :active_imagenes, :photo_imagenes, :id_productos, :extra_imagenes)";
				$stmt = $db->prepare($sql);
				$stmt->bindParam(':name_imagenes', $name_imagenes, PDO::PARAM_STR);
				$stmt->bindParam(':active_imagenes', $active_imagenes, PDO::PARAM_STR);
				$stmt->bindParam(':photo_imagenes', $photo_imagenes, PDO::PARAM_STR);
	...
	public function orden($id_imagenes, $orden_imagenes){
		global $db;
		try{
			$sql = "UPDATE productos_imagenes SET orden_imagenes = :orden_imagenes WHERE id_imagenes = :id_imagenes";
			$stmt = $db->prepare($sql);
			$stmt->bindParam(':id_imagenes', $id_imagenes, PDO::PARAM_STR);
	...
	public function delete($id_imagenes){
		...
		$sql = "UPDATE productos_imagenes SET eliminado_imagenes = :eliminado_imagenes  WHERE id_imagenes = :id_imagenes";
		$stmt = $db->prepare($sql);
		$stmt->bindParam(':id_imagenes', $id_imagenes, PDO::PARAM_STR);
...
$productos_imagenes = new productos_imagenes();