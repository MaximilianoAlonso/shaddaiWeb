...
class cursos_imagenes {
	private $db;

	public function tabla($id_cursos = 0, $requestData, $web){
		global $db, $sesion;
		try{
			...
			while($row = $q->fetchObject()) {
				...
				$dir = _HOSTDIR_."/uploads/cursos/imagenes/big/".$row->photo_imagenes;
				$dir2 = _HOSTDIR_."/uploads/cursos/imagenes/small/".$row->photo_imagenes;
				if (file_exists($dir) && file_exists($dir2) && $row->photo_imagenes != "") {
					$nestedData[] = '<img src="'._HOST_.'/uploads/cursos/imagenes/small/'.$row->photo_imagenes.'" alt="'.$row->photo_imagenes.'" style="max-width: 150px;" >';
				}else{
	...
	public function edit($id_imagenes){
		global $db, $sesion;
		try{
			$sql= "SELECT * FROM cursos_imagenes WHERE id_imagenes=:id_imagenes AND eliminado_imagenes = 0 ";
			$sql = $db->prepare($sql);
			...
			if($sql->rowCount() > 0){
				while($row = $sql->fetchObject()) {
					...
					$dir2 = _HOSTDIR_."/uploads/cursos/imagenes/small/".$row->photo_imagenes;
					if (file_exists($dir) && file_exists($dir2) && $row->photo_imagenes != "") {
						$this->photo_imagenes=_HOST_."/uploads/cursos/imagenes/small/".$row->photo_imagenes;
					}else{
	...
	public function select($id_imagenes = null, $id_cursos = -1){
		global $db;
		try{
			$data = '';
			$sql = " SELECT * FROM cursos_imagenes WHERE eliminado_imagenes = 0 ";
			if ($id_cursos >= 0) {
	...
	public function data($id_cursos = 0, $random = null, $limit = 0){
		global $db, $sesion;
		try{
			$data = array();
			$sql = "SELECT * FROM cursos_imagenes WHERE active_imagenes = '1' AND eliminado_imagenes = 0 ";
			if ($id_cursos > 0) {
				$sql .= " AND id_cursos = '".$id_cursos."' ";
			}
			if ($random == true) {
			...
			if($sql->rowCount() > 0){
				while($row = $sql->fetchObject()) {
					...
					if (file_exists($dir)  && $row->photo_imagenes != "") {
						$json['photo'] =_HOST_."/uploads/cursos/imagenes/big/".$row->photo_imagenes;
					}else{
					...
					if (file_exists($dir2)  && $row->photo_imagenes != "") {
						$json['photo_small'] =_HOST_."/uploads/cursos/imagenes/small/".$row->photo_imagenes;
					}else{
						$json['photo_small'] =_HOST_."/uploads/no-disponible.jpg";
					}

					$json['cursos'] = $row->id_cursos;
	...
	public function listado($id_cursos = 0, $pagination = 0, $limit = 10){
		global $db, $sesion;
		try{
	...
	public function update($id_imagenes, $name_imagenes, $active_imagenes, $photo_imagenes, $id_cursos, $extra_imagenes){
		global $db, $sesion;
		try{
			$sql = "UPDATE cursos_imagenes SET name_imagenes = :name_imagenes, active_imagenes = :active_imagenes, photo_imagenes = :photo_imagenes, id_cursos = :id_cursos, extra_imagenes = :extra_imagenes WHERE id_imagenes = :id_imagenes";
			$stmt = $db->prepare($sql);
	...
	public function add($name_imagenes, $active_imagenes, $photo_imagenes, $id_cursos, $extra_imagenes){
		global $db, $sesion;
		try{

			$sql = "INSERT INTO cursos_imagenes (name_imagenes, active_imagenes, photo_imagenes, id_cursos, extra_imagenes) VALUES(:name_imagenes, :active_imagenes, :photo_imagenes, :id_cursos, :extra_imagenes)";
			$stmt = $db->prepare($sql);
			$stmt->bindParam(':name_imagenes', $name_imagenes, PDO::PARAM_STR);
			$stmt->bindParam(':active_imagenes', $active_imagenes, PDO::PARAM_STR);
			$stmt->bindParam(':photo_imagenes', $photo_imagenes, PDO::PARAM_STR);
	...
	public function orden($id_imagenes, $orden_imagenes){
		global $db;
		try{
			$sql = "UPDATE cursos_imagenes SET orden_imagenes = :orden_imagenes WHERE id_imagenes = :id_imagenes";
			$stmt = $db->prepare($sql);
			$stmt->bindParam(':id_imagenes', $id_imagenes, PDO::PARAM_STR);
	...
	public function delete($id_imagenes){
		...
		$sql = "UPDATE cursos_imagenes SET eliminado_imagenes = :eliminado_imagenes  WHERE id_imagenes = :id_imagenes";
		$stmt = $db->prepare($sql);
		$stmt->bindParam(':id_imagenes', $id_imagenes, PDO::PARAM_STR);
...
$cursos_imagenes = new cursos_imagenes();