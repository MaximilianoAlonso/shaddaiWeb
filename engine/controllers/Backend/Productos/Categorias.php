...
class Categorias extends App\Controllers{
	...
	public function __invoke(Request $request, Response $response, $args){
		...
		global $productos_categorias;

		$content['v_p'] = $content['v_p_2']  = 'active';

		$content['modulo'] = "Categorias";

		$content['sector'] = "Categorias";
		$content['back'] = _HOST_.'/admin/productos/categorias';
		...
		if(isset($_GET['tabla'])){
			if(isset($_POST['order'])){
				$requestData = $_POST;
				echo $productos_categorias->tabla($requestData, $content['back']);
		...
		if(isset($_GET['del'])){
			if(isset($_POST['del_id'])){
				echo $productos_categorias->delete($_POST['del_id']);
				return $response->withHeader('Content-type', 'application/json');
			}
		}
		...
		$response = $this->ci->view->render($response, 'backend/productos/categorias.twig', ['content' => $content, 'cproductos' => $cproductos, 'args' => $args, 'return_data' => $return_data]);
		return $response;
	...
	public function add(Request $request, Response $response, $args){
		...
		global $productos_categorias, $productos_lineas, $functions;
		...
		if(isset($_POST['btn-save'])){
			...
			if(isset($_POST['photo_categorias']) && $_FILES['fotos']['name'] != ""){
				...
				$path = _HOSTDIR_."/uploads/productos/categorias/";
				$fotos = $_FILES['fotos'];
			...
			if($productos_categorias->add($name_categorias, $seo_categorias, $active_categorias, $photo_categorias, $id_lineas, $extra)){
				$this->functions->redirect(_HOST_."/admin/productos/categorias?n");
				return $response;
			}else{
		...
		$response = $this->ci->view->render($response, 'backend/productos/categorias.twig', ['content' => $content, 'cproductos' => $cproductos, 'args' => $args, 'return_data' => $return_data, 'productos_lineas' => $productos_lineas]);
		return $response;
	...
	public function edit(Request $request, Response $response, $args){
		...
		if(isset($_POST['btn-save'])){
			...
			if($productos_categorias->update($args['id'], $name_categorias, $seo_categorias, $active_categorias, $photo_categorias, $id_lineas, $extra)){
				$return_data['type'] = 'success';
		...
		if(!$productos_categorias->edit($args['id'])){
		...
		$response = $this->ci->view->render($response, 'backend/productos/categorias.twig', ['content' => $content, 'cproductos' => $cproductos, 'args' => $args, 'return_data' => $return_data, 'productos_categorias' => $productos_categorias, 'productos_lineas' => $productos_lineas]);
		return $response;
	}
		...
		if(isset($_POST['btn-save'])){
			...
			if (count($orden) > 0) {
				foreach ($orden as $key => $value) {
					$productos_categorias->orden($value['id'], $key);
				}
				$return_data['type'] = 'success';
				$return_data['msje'] = 'Se ha actualizado correctamente.';
			}else{
