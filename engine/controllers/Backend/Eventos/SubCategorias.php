...
class SubCategorias extends App\Controllers{
	...
	public function __invoke(Request $request, Response $response, $args){
		...
		global $eventos_subcategorias;

		$content['v_e'] = $content['v_e_3']  = 'active';

		$content['modulo'] = "Sub-Categorias";

		$content['sector'] = "Sub-Categorias";
		$content['back'] = _HOST_.'/admin/eventos/subcategorias';
		...
		if(isset($_GET['tabla'])){
			if(isset($_POST['order'])){
				$requestData = $_POST;
				echo $eventos_subcategorias->tabla($requestData, $content['back']);
		...
		if(isset($_GET['del'])){
			if(isset($_POST['del_id'])){
				echo $eventos_subcategorias->delete($_POST['del_id']);
				return $response->withHeader('Content-type', 'application/json');
			}
		}
		...
		$response = $this->ci->view->render($response, 'backend/eventos/subcategorias.twig', ['content' => $content, 'ceventos' => $ceventos, 'args' => $args, 'return_data' => $return_data]);
		return $response;
	...
	public function add(Request $request, Response $response, $args){
		...
		global $eventos_subcategorias, $eventos_categorias, $functions;
		...
		if(isset($_POST['btn-save'])){
			...
			if(isset($_POST['photo_subcategorias']) && $_FILES['fotos']['name'] != ""){
				...
				$path = _HOSTDIR_."/uploads/eventos/subcategorias/";
				$fotos = $_FILES['fotos'];
			...
			if($eventos_subcategorias->add($name_subcategorias, $seo_subcategorias, $active_subcategorias, $photo_subcategorias, $id_categorias, $extra)){
				$this->functions->redirect(_HOST_."/admin/eventos/subcategorias?n");
				return $response;
			}else{
		...
		$response = $this->ci->view->render($response, 'backend/eventos/subcategorias.twig', ['content' => $content, 'ceventos' => $ceventos, 'args' => $args, 'return_data' => $return_data, 'eventos_categorias' => $eventos_categorias]);
		return $response;
	...
	public function edit(Request $request, Response $response, $args){
		...
		if(isset($_POST['btn-save'])){
			...
			if($eventos_subcategorias->update($args['id'], $name_subcategorias, $seo_subcategorias, $active_subcategorias, $photo_subcategorias, $id_categorias, $extra)){
				$return_data['type'] = 'success';
		...
		if(!$eventos_subcategorias->edit($args['id'])){
		...
		$response = $this->ci->view->render($response, 'backend/eventos/subcategorias.twig', ['content' => $content, 'ceventos' => $ceventos, 'args' => $args, 'return_data' => $return_data, 'eventos_subcategorias' => $eventos_subcategorias, 'eventos_categorias' => $eventos_categorias]);
		return $response;
	}
		...
		if(isset($_POST['btn-save'])){
			...
			if (count($orden) > 0) {
				foreach ($orden as $key => $value) {
					$eventos_subcategorias->orden($value['id'], $key);
				}
				$return_data['type'] = 'success';
				$return_data['msje'] = 'Se ha actualizado correctamente.';
			}else{
